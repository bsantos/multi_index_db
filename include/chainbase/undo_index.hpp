...
namespace chainbase {
   ...
   class undo_index {
    public:
      ...
      struct node : detail::hook<Indices, Allocator>..., detail::value_holder<T> {
         using value_type = T;
         using allocator_type = Allocator;
         template<typename... A>
         explicit node(A&&... a) : detail::value_holder<T>{static_cast<A&&>(a)...} {}
      ...
      struct old_node : detail::hook<index0_type, Allocator>, detail::value_holder<T> {
         using value_type = T;
         using allocator_type = Allocator;
         template<typename... A>
         explicit old_node(const T& t) : detail::value_holder<T>{t} {}
    ...
    public:
      ...
      const auto& get() const { return std::get<detail::find_tag<Tag, Indices...>::value>(_indices); }
    ...
    private:
      ...
      void compress_impl(undo_state& session) noexcept {
         ...
         detail::remove_if_after_and_dispose(_old_values, _old_values.before_begin(), get_old_values_end(_undo_stack.back()),
         ...
         detail::remove_if_after_and_dispose(_removed_values, _removed_values.before_begin(), get_removed_values_end(_undo_stack.back()),
      ...
      void dispose(typename detail::list_base<old_node, index0_type>::iterator old_start, typename detail::list_base<node, index0_type>::iterator removed_start) noexcept {
